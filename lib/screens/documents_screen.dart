...
// Keep this aligned with the video formats accepted by Media Cutter. Files
// saved in Sonarpad Documents must remain media items instead of being opened
// by the text document reader.
const Set<String> _videoDocumentExtensions = {
  'mp4',
  'avi',
  'mov',
  'mkv',
...
const Set<String> _localMediaDocumentExtensions = {
  ..._audioDocumentExtensions,
  ..._videoDocumentExtensions,
};
...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() =>
            _errorMessage = AppLocalizations.of(context).libraryLoadError(AppLocalizations.of(context).technicalErrorGeneric));
      }
    } finally {
  ...
  Future<void> _pickFile() async {
    ...
    try {
      ...
      if (mounted) {
        _showSnack(AppLocalizations.of(context).fileOpenError(AppLocalizations.of(context).technicalErrorGeneric));
      }
      return;
    }
    ...
    for (final file in result.files) {
      ...
      if (f.path.toLowerCase().endsWith('.zip')) {
        try {
          ...
          if (mounted) {
            _showSnack(AppLocalizations.of(context).importZipError(AppLocalizations.of(context).technicalErrorGeneric));
          }
        }
        continue;
      }
      ...
      try {
        ...
        if (mounted) {
          _showSnack(AppLocalizations.of(context).documentAddError(AppLocalizations.of(context).technicalErrorGeneric));
        }
        continue;
      }
    }
  ...
  Future<void> _remove(String id) async {
    ...
    try {
      ...
      if (mounted) _showSnack(l10n.documentRemoveError(l10n.technicalErrorGeneric));
      return;
    }
    if (mounted) {
      setState(() {});
  ...
  Future<void> _shareLocalMediaDocument(DocumentItem doc) async {
    ...
    try {
      ...
      dev.log('DocumentsScreen: errore condivisione media: $e');
      if (mounted) {
        _showSnack(l10n.error(l10n.technicalErrorGeneric));
      }
    }
  ...
  Future<void> _exportDocument(DocumentItem doc) async {
    ...
    try {
      ...
      if (mounted) {
                showStatusMessage(context, '${AppLocalizations.of(context).exportError}: ${AppLocalizations.of(context).technicalErrorGeneric}');
      }
    }
  }
  ...
  Future<String> _exportTextForDocument(DocumentItem doc) async {
    ...
    if (result.text.trim().isEmpty) {
      throw Exception(result.error == null ? l10n.noExportableTextFound : l10n.technicalErrorGeneric);
    }
    return result.text;
  }
  ...
  Future<void> _openDocument(DocumentItem doc) async {
    ...
    if (_isLibrivoxDocument(doc)) {
      try {
        ...
        if (mounted) _showSnack(AppLocalizations.of(context).error(AppLocalizations.of(context).technicalErrorGeneric));
      }
      return;
    }
    ...
    if (_isLocalMediaDocument(doc)) {
      try {
        final resolvedPath = await _service.resolveFilePath(doc);
        final isVideo = _isLocalVideoDocument(doc);
        if (!mounted) return;
        final episode = PodcastEpisode(
          title: doc.displayName,
          description: doc.displayName,
          audioUrl: Uri.file(resolvedPath).toString(),
          id: 'document_media:${doc.id}',
        );
        await Navigator.of(context).push(
          MaterialPageRoute<void>(
            settings: const RouteSettings(name: '/documents/media_player'),
  ...
  bool _isLocalVideoDocument(DocumentItem doc) {
    final ext = doc.extension.toLowerCase();
    return _videoDocumentExtensions.contains(ext);
  }

  bool _isLocalMediaDocument(DocumentItem doc) {
    final ext = doc.extension.toLowerCase();
    return _localMediaDocumentExtensions.contains(ext);
  }
