...
class Gs1ParsedData {
  ...
  bool get hasValidAic => aic != null && aic!.length == 9 && aic!.startsWith('0') && RegExp(r'^\d+$').hasMatch(aic!);
...
class Gs1Parser {
  ...
  static Gs1ParsedData parse(String rawValue) {
    ...
    if (gtin == null && aic == null && (data.length == 13 || data.length == 8) && RegExp(r'^\d+$').hasMatch(data)) {
      gtin = data;
    }
    ...
    return Gs1ParsedData(gtin: gtin, aic: aic, batch: batch, expiry: expiry, serial: serial);
  }
}
