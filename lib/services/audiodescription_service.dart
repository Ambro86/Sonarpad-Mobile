...
class AudiodescriptionItem {
  ...
  static String _formatDate(dynamic timestamp) {
    ...
    try {
      final ms = (timestamp is int) ? timestamp * 1000 : int.parse(timestamp.toString()) * 1000;
...
class AudiodescriptionGroup {
  ...
  static String _normalizeTitle(String title) {
    ...
    if (lower.contains('film - audiodescrizioni') || (title.startsWith('Film (') && title.endsWith(')'))) {
      return 'Film';
    }
...
class AudiodescriptionService {
  static const _listUrlB64 = "GhUdXRJPS0YdExZHSggBDBwNBxIMXwIaCh0KHBVHTg4YSygCEBMGFVdaNwYBExMZTAVYMAYAHhJGXwQTF0YHFwANXk4YBQABXQYMQwQHBR0KFk4FWAIQSQUGARVHSA8WSgMcHQ8=";
  static const _catalogUrlB64 = "GhUdXRJPS0YdExZHSggBDBwNBxIMXwIaCh0KHBVHTg4YSygCEBMGFVdaNwYBExMZTAVYMAYAHhJGXwQTF0YHFwANXk4YBQABXQYMQwQHBR0KFk4FWAIQSQoOBgAFQgYAAUcKHAJHRxIaCg==";
  static const _urlKey = "rai-audio";

  final http.Client _client;

  AudiodescriptionService({http.Client? client}) : _client = client ?? http.Client();
  ...
  Future<String> _fetchAndDecodeLucePayload(String url, String secretKey) async {
    final resp = await _client.get(Uri.parse(url), headers: {
      'User-Agent': 'SonarpadMobile/0.1',
    });
    if (resp.statusCode != 200) throw Exception('Errore di rete: ${resp.statusCode}');
  ...
  Future<List<AudiodescriptionItem>> fetchRecentCatalog(String secretKey) async {
    if (secretKey.trim().isEmpty) throw Exception('Codice segreto Luce mancante.');

    final url = _decodeObfuscatedUrl(_listUrlB64, _urlKey);
    final rawJson = await _fetchAndDecodeLucePayload(url, secretKey);
    final list = jsonDecode(rawJson) as List<dynamic>;

    final items = list.map((e) => AudiodescriptionItem.fromJson(e)).where((e) => e.audioUrl.isNotEmpty).toList();
    // In Rust it is sorted by added date descending
    return items;
  }

  Future<List<AudiodescriptionGroup>> fetchGroupedCatalog(String secretKey) async {
    if (secretKey.trim().isEmpty) throw Exception('Codice segreto Luce mancante.');
    ...
    final groups = list.map((e) => AudiodescriptionGroup.fromJson(e)).where((g) => g.items.isNotEmpty).toList();
    ...
    final result = merged.entries.map((e) => AudiodescriptionGroup(title: e.key, items: e.value)).toList();
