...
class AppLocalizationsZh extends AppLocalizations {
  ...
  String get convertMediaOutputNotWritable =>
      '无法直接访问所选文件夹。文件将保存在 Sonarpad 的内部文件夹中；转换完成后，你可以分享文件，或将其保存到“文件”应用中。';
  ...
  String get infoDescription =>
      'Sonarpad 是一款简单但功能丰富的应用。它专为盲人和视障人士使用 VoiceOver 进行无障碍操作而设计，可用于收听新闻、搜索并订阅播客、导入维基百科文章、将文档添加到资料库并进行保存和编辑。Sonarpad 会持续更新，每项功能都旨在让日常生活更轻松。';
  ...
  String positionLabel(int position, String targetName) {
    return '位置 $position：位于 $targetName 之前';
  }
  ...
  String rssImportComplete(int count) {
    return '已导入 RSS 来源：$count';
  }

  @override
  String rssImportError(Object error) {
    return 'RSS 导入错误：$error';
  }
  ...
  String chunkCreated(int index, int total) {
    return '已创建第 $index/$total 个分段。正在朗读...';
  }

  @override
  String playingChunk(int index, int total, int size) {
    return '正在播放第 $index/$total 个分段（$size 字节）...';
  }

  @override
  String readingFinished(int readyChunks, int totalChunks, String libraryPath) {
    return '朗读结束。已创建分段：$readyChunks/$totalChunks。资料库：$libraryPath';
  }
  ...
  String edgeTtsError(Object error) {
    return 'Edge TTS 错误：$error';
  }

  @override
  String audioChunksReady(int readyChunks, int totalChunks) {
    return '音频分段已就绪：$readyChunks / $totalChunks';
  }
  ...
  String sharedDocumentsImportComplete(int count) {
    return '从 iTunes / Apple 设备导入的文档：$count';
  }

  @override
  String libraryLoadError(Object error) {
    return '资料库加载错误：$error';
  }
  ...
  String get settingsReadingSleepTimerHint =>
      '在所选时间后自动停止朗读当前文档并保存停止位置。每次开始朗读文档时都会重新开始倒计时。';
  ...
  @override
  String get tvRecordingChooseDay => '选择日期';
  ...
  String settingsCodeRequestBody(
    ...
    return '名字：$name；姓氏：$surname；电子邮件：$email；操作系统：$os';
  }
  ...
  String whatIsNewInVersion(String version) {
    return '版本 $version 的更新内容';
  }

  @override
  String changelogLoadError(Object error) {
    return '加载更新内容时出错：$error';
  }
  ...
  String get settingsIncludeFootnotesInTextHint =>
      '对于支持的 EPUB 图书，在引用脚注的段落后立即显示相应注释。';
  ...
  String get settingsMultipleDocumentBookmarksHint =>
      '关闭后，每个文档只保留一个书签。启用后，可以在同一文档中保存多个书签。';
  ...
  String documentBookmarkChoiceLabelWithPreview(
    ...
    return '书签 $order，第 $paragraph 段。$preview';
  }
  ...
  String get settingsSonarTubeAutoplayHint =>
      '视频或音轨结束后，如果有下一项，SonarTube 会自动播放下一项。';
  ...
  String get mediaCutterPartsHint =>
      '轻点片段可试听。已删除的片段会从列表中消失，播放时会跳过，也不会被保存。音频效果只会在保存媒体时应用到整个片段。';
  ...
  String mediaCutterPartDeleted(String start, String end) {
    return '已删除从 $start 到 $end 的片段。';
  }

  @override
  String mediaCutterPartRestored(String start, String end) {
    return '已恢复从 $start 到 $end 的片段。';
  }
  ...
  String get mediaCutterAdvancedModeDescription =>
      '参考常见媒体编辑程序设计。可以将媒体文件分成多个片段，并删除不需要的部分。';
  ...
  String get newsAddCommunityInstructions =>
      '输入来源标题以及 RSS 源网址或网站网址。Sonarpad 会使用所选新闻语言；如果输入的是网站网址，还会尝试自动查找订阅源。';
  ...
  String get developerUseExperimentalFlutterRendererHint =>
      '暂时停用 UIKit，以比较 VoiceOver 在纯 Flutter 下的表现。';
  ...
  String get radioScheduleOpenRequirement =>
      '在你浏览 Sonarpad 的其他屏幕时，计划录制仍会继续工作。Sonarpad 必须保持打开；如果应用被关闭或被系统挂起，则无法保证录制会按时开始。';
  ...
  String radioScheduledRecordingRangeWithTitle(
    ...
    return '计划录制：$start - $end。标题：$title。';
  }
  ...
  @override
  String get sonarTubeChannelPlaylists => '播放列表';
  ...
  @override
  String get audioDescriptionHighDemandMessage => '多次尝试后 AI 服务仍不可用。是否继续等待并重试？';

  @override
  String get audioDescriptionContinueWaiting => '继续等待';

  @override
  String get audioDescriptionStopGeneration => '停止';
